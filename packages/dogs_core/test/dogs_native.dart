...
void main() {
  test("from native coercion", () {
    ...
    expect(
        DogList([DogInt(1), DogInt(2)]).value, containsAll(codec.fromNative([1, 2]).asList!.value));
  ...
  test("to native coercion", () {
    ...
    expect(true, ListEquality().equals([1, 2], DogList([DogInt(1), DogInt(2)]).coerceNative()));
    expect(
        true,
        MapEquality().equals({"a": 1, "b": 2},
            DogMap({DogString("a"): DogInt(1), DogString("b"): DogInt(2)}).coerceNative()));
  });
}
