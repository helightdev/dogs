...
abstract class QueryableRepository<T extends Object, ID extends Object> {
  ...
  Future<List<T>> findAllByQuery(QueryLike query, [Sorted sort = const Sorted.empty()]);

  Future<T?> findOneByQuery(QueryLike query, [Sorted sort = const Sorted.empty()]);
...
mixin QueryableRepositoryMixin<T extends Object, ID extends Object, SYS extends OdmSystem, SYS_DB extends QueryableDatabase, SYS_ID extends Object>
  on Repository<T, ID> implements QueryableRepository<T, ID> {

  @override
  Future<List<T>> findAllByQuery(QueryLike query, [Sorted sort = const Sorted.empty()]) {
    return (database as QueryableDatabase<T,SYS_ID>).findAllByQuery(query.asQuery, sort);
  }

  @override
  Future<T?> findOneByQuery(QueryLike query, [Sorted sort = const Sorted.empty()]) {
    return (database as QueryableDatabase<T,SYS_ID>).findOneByQuery(query.asQuery, sort);
  }

  @override
  Future<int> countByQuery(QueryLike query) {
    return (database as QueryableDatabase<T,SYS_ID>).countByQuery(query.asQuery);
  }

  @override
  Future<bool> existsByQuery(QueryLike query) {
    return (database as QueryableDatabase<T,SYS_ID>).existsByQuery(query.asQuery);
  }

  @override
  Future<void> deleteOneByQuery(QueryLike query) {
    return (database as QueryableDatabase<T,SYS_ID>).deleteOneByQuery(query.asQuery);
  }

  @override
  Future<void> deleteAllByQuery(QueryLike query) {
    return (database as QueryableDatabase<T,SYS_ID>).deleteAllByQuery(query.asQuery);
  }
}
...
class Query extends QueryLike {
  ...
  const Query.cursor(int limit, int skip) : this(null, limit: limit, skip: skip);
...
class Sorted {
    ...
    factory Sorted.byFields(List<String> fields, {List<bool>? ascending, defaultAscending = true}) {
      ascending ??= List.filled(fields.length, defaultAscending);
      if (fields.length != ascending.length) throw Exception("Fields and ascending must have the same length.");
      var sorts = <SortExpr>[];
...
class FilterAll extends FilterExpr {
  final String field;
  final List<Object?> values;
  const FilterAll(this.field, this.values);
...
class FilterAny extends FilterExpr {
  final String field;
  final FilterExpr filter;
  const FilterAny(this.field, this.filter);
