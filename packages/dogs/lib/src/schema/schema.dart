...
class DogSchema {
  ...
  MapEntry<String, APISchemaObject> getStructureSchema(DogStructure structure) {
    ...
    var properties = Map.fromEntries(structure.fields.map((e) {
      if (!e.structure) {
        ...
        var serialSchema = APISchemaObject.empty()
          ..type = serialType;

        if (e.iterableKind == IterableKind.none) {
          var object = serialSchema;
          if (e.optional) object.isNullable = true;
          e.metadataOf<APISchemaObjectMetaVisitor>().forEach((element) { element.visit(object); });
    ...
    structure.metadataOf<APISchemaObjectMetaVisitor>().forEach((element) { element.visit(value); });
  ...
  APIDocument getApiDocument() {
    ...
    document.paths = {};
    document.info = APIInfo("DOG Mockup", "1.0", description: "Autogenerated component mockup of all registered structures which are not synthetic.");
